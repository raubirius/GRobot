...
// a slúži na generovanie svetelného náteru tvoreného viacerými zdrojmi
// svetla. Licencia a zdroje sú uvedené nižšie v anglickom jazyku.
// 
 ...
 * @exclude
 */
public class LightMixerPaint extends AbstractLightPaint
{
	/**
	 * <p>The paint context of the {@link LightMixerPaint
	 * LightMixerPaint} class.</p>
	 * 
	 * @see PaintContext
	 */
	public class LightMixerPaintContext extends AbstractLightPaintContext
	{
		/**
			...
			else
			{
				...
				for (int j = 0, i = 0; j < h; ++j)
					for (int k = 0; k < w; ++k, ++i)
					{
						...
						int /*min;
						int max =*/ min = computingStack[0][2] = dims[0] *
							(xx * xx + yy * yy) + dilutions[0];

						// Compute Dₘ and find min:
						for (int l = 1; l < lightsCount; ++l)
						{
							...
							if (min > computingStack[l][2])
								min = computingStack[l][2];
						}
						...
						{*/
							...
							for (int l = 0; l < lightsCount; ++l)
							{
								// Σ += /*max*/min / computingStack[l][2];
								b += colors[l][0] * /*max*/min /
									computingStack[l][2];
								g += colors[l][1] * /*max*/min /
									computingStack[l][2];
								r += colors[l][2] * /*max*/min /
									computingStack[l][2];
								a += colors[l][3] * /*max*/min /
									computingStack[l][2];
							}
	...
	/**
	 * <p>Creates and returns the {@link LightMixerPaintContext
	 * LightMixerPaintContext} instance that will be used to
	 ...
	 * @return the {@linkplain LightMixerPaintContext arbitrary
	...
	{ return new LightMixerPaintContext(); }
}
